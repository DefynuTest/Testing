...
<head>
    ...
    <spring:url value="/resources/css/track.css" var="trackCSS" />
	...
	<link href="${trackCSS}" rel="stylesheet" />
...
<c:otherwise>
                ...
                <div class="col-md-2">
                    ...
                    <figure>
                        <br><br>
                          ...
                          <p><i><b>ORDER ID:&nbsp;${orderid[status.index]}</b></i></p>
  ...
  <c:if test="${Status[status.index] .equals('ND')}">
 
 <div id="qty" >
		<form   action="/cancel?orderid=${orderid[status.index]}" method="POST">
 
			 <button class="btn btn-primary btn-sm" id="cancel" type="submit" formmethod="POST" formaction="/cancel?orderid=${orderid[status.index]}">Cancel</button>
</form>
	</div>
	</c:if>
			<span id="price" style="color:black;font-weight:bold">INR:</span>
  ...
  <div class="progress">
  <c:choose>
  <c:when test="${Status[status.index] .equals('ND')}">
 ...
 <div class=" "><label>Bag Discount</label>&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="">- Rs. 1,369 </span></div><br>
	...
	<br><br>
	<form align="right" action="/confrimorder" method="get" target="_self">
